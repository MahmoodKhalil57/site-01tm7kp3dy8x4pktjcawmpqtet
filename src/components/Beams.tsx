...
const CanvasWrapper: FC<{ children: ReactNode }> = ({ children }) => {
  ...
  useEffect(() => {
    // Double-rAF gives the Canvas ResizeObserver time to fire and set
    // the correct dimensions before we make the canvas visible.
...
const Beams: FC<BeamsProps> = ({
  ...
  const [bgHex, setBgHex] = useState(() =>
    cssVarToHex(backgroundColor, "#000000"),
  );
  const [fgHex, setFgHex] = useState(() =>
    cssVarToHex(forgroundColor, "#7c3aed"),
  );

  const resolveColors = useCallback(() => {
    setBgHex(cssVarToHex(backgroundColor, "#000000"));
    setFgHex(cssVarToHex(forgroundColor, "#7c3aed"));
  }, []);
  ...
  vec3 getNormal(vec3 pos) {
    ...
    gl_FragColor.rgb -= randomNoise / 15. * uNoiseIntensity;`,
        ...
        material: { fog: true },
        uniforms: {
          diffuse: new THREE.Color(...hexToNormalizedRGB(bgHex)),
    ...
    [bgHex, speed, noiseIntensity, scale],
  );
    ...
    <CanvasWrapper>
      <group rotation={[0, 0, degToRad(rotation)]}>
        <PlaneNoise
          ...
          height={beamHeight}
        />
        <DirLight color={fgHex} position={[0, 3, 10]} />
      </group>
