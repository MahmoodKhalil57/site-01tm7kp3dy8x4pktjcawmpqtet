...
// --- CSS variable resolution (matches Beams.tsx pattern) ---
...
const Waves: React.FC<WavesProps> = ({
  ...
  const [resolvedLineColor, setResolvedLineColor] = useState(
    () => lineColorProp ?? cssVarToHex(CSS_VAR_PRIMARY, "#7c3aed"),
  );
  const [resolvedBgColor, setResolvedBgColor] = useState(
    () => backgroundColorProp ?? cssVarToHex(CSS_VAR_BACKGROUND, "#000000"),
  );

  const resolveColors = useCallback(() => {
    setResolvedLineColor(
      lineColorProp ?? cssVarToHex(CSS_VAR_PRIMARY, "#7c3aed"),
    );
    setResolvedBgColor(
      backgroundColorProp ?? cssVarToHex(CSS_VAR_BACKGROUND, "#000000"),
    );
  }, [lineColorProp, backgroundColorProp]);

  // Re-resolve on theme change (dark/light toggle)
  useEffect(() => {
    resolveColors(); // eslint-disable-line react-hooks/set-state-in-effect
  ...
  const lineColor = resolvedLineColor;
  const backgroundColor = resolvedBgColor;
  ...
  return (
    ...
    >
      <div
        className="absolute top-0 start-0 bg-primary rounded-full w-2 h-2 hidden"
        style={{
          transform:
